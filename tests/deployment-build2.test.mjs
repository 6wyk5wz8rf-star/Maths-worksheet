...
test('release v2 uses a product-scoped cache namespace', async () => {
  const worker = await readProjectFile('service-worker.js');
  assert.match(worker, /const CACHE_PREFIX = 'maths-page-studio-';/);
  assert.match(worker, /const CACHE = `\$\{CACHE_PREFIX\}release-v2`;/);
...
test('release v2 addresses the complete changed asset and module graph', async () => {
